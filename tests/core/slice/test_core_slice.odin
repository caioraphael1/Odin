...
test_sort_by_indices :: proc(t: ^testing.T) {
	...
	for test_size in test_sizes {
...

		{
			...
			defer _ = delete_slice(sorted_indices)
...
test_unique :: proc(t: ^testing.T) {
	...
	for _ in 0..<10_000 {
		...
		_ = delete_slice(assorted)
		_ = delete_slice(original)
		_ = delete_slice(expected)
	}
}
