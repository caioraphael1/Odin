...
test_marshalling :: proc(t: ^testing.T) {
	{
		...
		defer {
			_ = delete_slice(f.child.dyn)
			_ = delete_slice(f.child.mappy)
	...
	"small_onetwenty": -18446744073709551615
		...
		defer {
			_ = delete_slice(backf.str)
			_ = delete_slice(backf.cstr)
			cbor.destroy(backf.value)
			_ = delete_slice(backf.base64)

			for e in backf.child.dyn { _ = delete_slice(e) }
			_ = delete_slice(backf.child.dyn)

			for k in backf.child.mappy { _ = delete_slice(k) }
			_ = delete_slice(backf.child.mappy)

			_ = delete_slice(backf.my_bytes)
...
test_unmarshal_map_into_struct_partially :: proc(t: ^testing.T) {
	...
	_ = delete_slice(more_bin)
	_ = delete_slice(less_out.bar.hello)
	_ = delete_slice(less_out.bar.world)
}
