...
test_bit_array_bias :: proc(t: ^testing.T) {
    for bias in -ELEM_BIT_SIZE..=ELEM_BIT_SIZE {
        ...
        for i in list {
            _ = bit_array.set(ba, i, true, allocator)
...
test_bit_array_biased_max_index :: proc(t: ^testing.T) {
    for bias in -ELEM_BIT_SIZE..=ELEM_BIT_SIZE {
        for max_index in 1+bias..<ELEM_BIT_SIZE {
            ...
            _ = bit_array.set(ba, max_index - 1, true, allocator)
...
test_bit_array_shrink :: proc(t: ^testing.T) {
    for bias in -ELEM_BIT_SIZE..=ELEM_BIT_SIZE {
        ...
        for i in 0..=N {
            biased_i := bias + i
            _ = bit_array.set(ba, biased_i, true, allocator)
            ...
            if i >= ELEM_BIT_SIZE {
                ...
                _ = bit_array.set(ba, bias, true, allocator)
                bit_array.shrink_dynamic_array(ba)
...
test_bit_array :: proc(t: ^testing.T) {
    ...
    defer {
        _ = delete_slice(list_set)
        _ = delete_slice(seen_set)
        _ = delete_slice(list_unset)
        _ = delete_slice(seen_unset)
    }
    ...
    for i in 1..=16 {
        for j in -1..=1 {
            n := ELEM_BIT_SIZE * i + j
            _ = bit_array.set(ba, n, true, allocator)
            _ = append(&list_set, n)
        }
    }
    ...
    // Test clearing.
    bit_array.clear_dynamic_array(ba)
    iter = bit_array.make_iterator(ba)
    for i in 0..=MAX_INDEX {
        testing.expectf(t, bit_array.get(ba, i) == false,
