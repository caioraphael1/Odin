...
gb_internal lbValue lb_emit_union_cast(lbProcedure *p, lbValue value, Type *type, TokenPos pos) {
    ...
    if (!is_tuple) {
        if (!build_context.no_type_assert) {
            ...
            char const *name = "__type_assertion_check2";
...
gb_internal lbValue lb_cstring_len(lbProcedure *p, lbValue value) {
    ...
    return lb_emit_internal_call(p, "__cstring_len", args);
}
gb_internal lbValue lb_cstring16_len(lbProcedure *p, lbValue value) {
    ...
    return lb_emit_internal_call(p, "__cstring16_len", args);
}
    ...
    lb_begin_procedure_body(dispose_proc);
    {
        for (isize object_index : objc_object_indices) {
            ...
            // Copy helper
            {
                ...
                lb_emit_internal_call(copy_proc, "__Block_object_assign", copy_args);
            }
            ...
            // Dispose helper
            {
                ...
                lb_emit_internal_call(dispose_proc, "__Block_object_dispose", dispose_args);
            }
        }
    }
...
gb_internal lbValue lb_handle_objc_block(lbProcedure *p, Ast *expr) {
    ...
    const bool  is_global             = capture_arg_count == 0;
    ...
    {
        // Reserve 1 extra arguments for: Indirect return values.
        auto call_args = array_make<LLVMValueRef>(temporary_allocator(), 0, user_proc.param_count + 1);
    ...
    lbValue isa_val      = lb_find_internal_value(m, is_global ? str_lit("_NSConcreteGlobalBlock") : str_lit("_NSConcreteStackBlock"));
...
gb_internal lbValue lb_handle_objc_register_selector(lbProcedure *p, Ast *expr) {
    ...
    lbValue ptr = lb_emit_internal_call(p, "__sel_registerName", args);
...
gb_internal lbValue lb_handle_objc_register_class(lbProcedure *p, Ast *expr) {
    ...
    lbValue ptr = lb_emit_internal_call(p, "__objc_allocateClassPair", args);
    lb_addr_store(p, dst, ptr);
...
gb_internal lbValue lb_handle_objc_send(lbProcedure *p, Ast *expr) {
    ...
    default:
        ...
        break;
    case ObjcMsg_normal: the_proc = lb_lookup_internal_procedure(m, str_lit("objc_msgSend"));        break;
    case ObjcMsg_fpret:  the_proc = lb_lookup_internal_procedure(m, str_lit("objc_msgSend_fpret"));  break;
    case ObjcMsg_fp2ret: the_proc = lb_lookup_internal_procedure(m, str_lit("objc_msgSend_fp2ret")); break;
    case ObjcMsg_stret:  the_proc = lb_lookup_internal_procedure(m, str_lit("objc_msgSend_stret"));  break;
    }
...
gb_internal lbValue lb_handle_objc_auto_send(lbProcedure *p, Ast *expr, Slice<lbValue> const arg_values) {
    ...
    if (!objc_super_orig_type) {
        ...
        switch (data.kind) {
            ...
            case ObjcMsg_fpret:
            case ObjcMsg_fp2ret:
                the_proc = lb_lookup_internal_procedure(m, str_lit("objc_msgSendSuper2"));
                break;
            case ObjcMsg_stret:
                the_proc = lb_lookup_internal_procedure(m, str_lit("objc_msgSendSuper2_stret"));
                break;
        }
    }
