...
void add_objc_proc_type(CheckerContext *c, Ast *call, Type *return_type, Slice<Type *> param_types) {
    ...
    try_to_add_package_dependency(c, "internal", "objc_msgSend");
    try_to_add_package_dependency(c, "internal", "objc_msgSend_fpret");
    try_to_add_package_dependency(c, "internal", "objc_msgSend_fp2ret");
    try_to_add_package_dependency(c, "internal", "objc_msgSend_stret");

    Slice<Ast *> args = call->CallExpr.args;
    if (args.count > 0 && args[0]->tav.objc_super_target) {
        try_to_add_package_dependency(c, "internal", "objc_msgSendSuper2");
        try_to_add_package_dependency(c, "internal", "objc_msgSendSuper2_stret");
    }
}
...
gb_internal bool check_builtin_objc_procedure(CheckerContext *c, Operand *operand, Ast *call, i32 id, Type *type_hint) {
    ...
    case BuiltinProc_objc_register_class: 
    {
        ...
        try_to_add_package_dependency(c, "internal", "objc_lookUpClass");
        try_to_add_package_dependency(c, "internal", "__sel_registerName");
        try_to_add_package_dependency(c, "internal", "__objc_allocateClassPair");
        return true;
    } break;
    ...
    case BuiltinProc_objc_block:
    {
        ...
        bool is_global_block = capture_arg_count == 0;
        if (is_global_block) {
            try_to_add_package_dependency(c, "internal", "_NSConcreteGlobalBlock");
        } else {
            try_to_add_package_dependency(c, "internal", "_NSConcreteStackBlock");
        }
...
gb_internal bool check_builtin_procedure(CheckerContext *c, Operand *operand, Ast *call, i32 id, Type *type_hint) {
        ...
        if (is_type_string(op_type) && id == BuiltinProc_len) {
            if (operand->mode == Addressing_Constant) {
                ...
                if (is_type_cstring(op_type)) {
                    add_package_dependency(c, "internal", "__cstring_len");
                } else if (is_type_cstring16(op_type)) {
                    add_package_dependency(c, "internal", "__cstring16_len");
                }
            }
    ...
    case BuiltinProc_type_info_of: {
        ...
        if (c->scope->flags&ScopeFlag_Global) {
            compiler_error("'type_info_of' Cannot be declared within the 'internal' package due to how the internals of the compiler works");
        }
        ...
        if (is_operand_value(o) && is_type_typeid(t)) {
            add_package_dependency(c, "internal", "__type_info_of");
    ...
    case BuiltinProc_typeid_of: {
        ...
        if (c->scope->flags&ScopeFlag_Global) {
            compiler_error("'typeid_of' Cannot be declared within the 'internal' package due to how the internals of the compiler works");
        }
        if (build_context.no_rtti) {
            error(call, "'%.*s' has been disallowed", LIT(builtin_name));
            return false;
    ...
    case BuiltinProc_abs: {
        ...
        if (operand->mode == Addressing_Constant) {
...

            {
                ...
                if (are_types_identical(bt, t_complex64))  add_package_dependency(c, "internal", "__complex64_abs");
                if (are_types_identical(bt, t_complex128)) add_package_dependency(c, "internal", "__complex128_abs");
                if (are_types_identical(bt, t_quaternion128)) add_package_dependency(c, "internal", "__quaternion128_abs");
                if (are_types_identical(bt, t_quaternion256)) add_package_dependency(c, "internal", "__quaternion256_abs");
            }
        }
    ...
    case BuiltinProc_raw_data:
        {
            ...
            if (!is_operand_value(x)) {
                gbString s = expr_to_string(x.expr);
                error(call, "'%.*s' expects a string, slice, or pointer to array type, got %s", LIT(builtin_name), s);
                gb_string_free(s);
                return false;
            }
            Type *t = base_type(x.type);
    ...
    case BuiltinProc_fixed_point_div_sat:
        {
            ...
            if (sz >= 64) {
                if (is_type_unsigned(x.type) || is_type_unsigned(y.type)) {
                    add_package_dependency(c, "internal", "__umodti3", true);
                    add_package_dependency(c, "internal", "__udivti3", true);
                } else {
                    add_package_dependency(c, "internal", "__modti3", true);
                    add_package_dependency(c, "internal", "__divti3", true);
                }
            }
