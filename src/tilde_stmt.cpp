...
gb_internal void cg_build_return_stmt_internal(cgProcedure *p, Slice<cgValue> const &results) {
    ...
    if (p->split_returns_index >= 0) {
        GB_ASSERT(p->type->Proc.calling_convention == ProcCC_Odin);
        ...
        GB_ASSERT_MSG(p->type->Proc.calling_convention != ProcCC_Odin, "missing %s", proc_calling_convention_strings[p->type->Proc.calling_convention]);
