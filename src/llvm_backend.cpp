...
gb_internal lbValue lb_simple_compare_hash(lbProcedure *p, Type *type, lbValue data, lbValue seed) {
    ...
    return lb_emit_internal_call(p, "__default_hasher", args);
}
...
gb_internal lbValue lb_hasher_proc_for_type(lbModule *m, Type *type) {
    ...
    if (type->kind == Type_Struct)  {
        ...
        args[0] = data;
        args[1] = seed;
        lbValue res = lb_emit_internal_call(p, "__default_hasher_cstring", args);
        lb_add_callsite_force_inline(p, res);
        LLVMBuildRet(p->builder, res.value);
        ...
        args[0] = data;
        args[1] = seed;
        lbValue res = lb_emit_internal_call(p, "__default_hasher_string", args);
        lb_add_callsite_force_inline(p, res);
        LLVMBuildRet(p->builder, res.value);
        ...
        args[0] = v;
        args[1] = seed;
        lbValue res = lb_emit_internal_call(p, "__default_hasher_f64", args);
        lb_add_callsite_force_inline(p, res);
        LLVMBuildRet(p->builder, res.value);
        ...
        args[1] = y;
        args[2] = seed;
        lbValue res = lb_emit_internal_call(p, "__default_hasher_complex128", args);
        lb_add_callsite_force_inline(p, res);
        LLVMBuildRet(p->builder, res.value);
        ...
        args[3] = w;
        args[4] = seed;
        lbValue res = lb_emit_internal_call(p, "__default_hasher_quaternion256", args);
        lb_add_callsite_force_inline(p, res);
        LLVMBuildRet(p->builder, res.value);
    } else {
...
gb_internal lbValue lb_map_get_proc_for_type(lbModule *m, Type *type) {
    ...
    {
        // __map_desired_position inlined
...
gb_internal lbValue lb_map_set_proc_for_type(lbModule *m, Type *type) {
...

    {
        ...
        lbValue grow_err_and_has_grown = lb_emit_internal_call(p, "__dynamic_map_check_grow", args);
    ...
    lb_start_block(p, insert_block);
    {
        ...
        lbValue result = lb_emit_internal_call(p, "__map_insert_hash_dynamic", args);
...
gb_internal lbValue lb_gen_map_key_hash(lbProcedure *p, lbValue const &map_ptr, lbValue key, lbValue *key_ptr_) {
    ...
    if (hashed_key.value == nullptr) {
        ...
        lbValue seed = {};
        {
            ...
            seed = lb_emit_internal_call(p, "__map_seed_from_map_data", args);
        }
...
gb_internal lbValue lb_internal_dynamic_map_get_ptr(lbProcedure *p, lbValue const &map_ptr, lbValue const &key) {
    ...
    if (build_context.dynamic_map_calls) {
        ...
        ptr = lb_emit_internal_call(p, "__dynamic_map_get", args);
    } else {
...
gb_internal void lb_internal_dynamic_map_set(lbProcedure *p, lbValue const &map_ptr, Type *map_type,
    ...
    if (build_context.dynamic_map_calls) {
        ...
        lb_emit_internal_call(p, "__dynamic_map_set", args);
    } else {
...
gb_internal lbValue lb_dynamic_map_reserve(lbProcedure *p, lbValue const &map_ptr, isize const capacity, TokenPos const &pos) {
    ...
    return lb_emit_internal_call(p, "__dynamic_map_reserve", args);
}
...
gb_internal void lb_register_objc_thing(
    ...
    if (g.class_impl_type != nullptr) {
        ...
        args.count = 1;
        args[0] = class_name;
        class_ptr = lb_emit_internal_call(p, call, args);
...
gb_internal void lb_finalize_objc_names(lbGenerator *gen, lbProcedure *p) {
    ...
    for (lbObjCGlobal g = {}; mpsc_dequeue(&gen->objc_selectors, &g); /**/) {
        lb_register_objc_thing(handled, m, args, class_impls, global_class_map, p, g, "__sel_registerName");
    }
    ...
    for (const auto &cd : class_impls) {
...

        {
            ...
            class_value = lb_emit_internal_call(p, "__objc_allocateClassPair", args);
        ...
        for (const ObjcMethodData &md : *methods) {
            ...
            args.count       = 1;
            args[0]          = class_value;
            meta_class_value = lb_emit_internal_call(p, "__object_getClass", args);
        ...
        for (const ObjcMethodData &md : *methods) {
            ...
            lb_emit_internal_call(p, "__class_addMethod", args);
        ...
        if (ivar_type != nullptr) {
            ...
            lb_emit_internal_call(p, "__class_addIvar", args);
        }

        // Complete the class registration
        args.count = 1;
        args[0] = class_value;
        lb_emit_internal_call(p, "__objc_registerClassPair", args);
    }
    ...
    for (auto const& kv : ivar_map) {
        ...
        lbValue ivar = lb_emit_internal_call(p, "__class_getInstanceVariable", args);

        args.count = 1;
        args[0] = ivar;
        lbValue ivar_offset     = lb_emit_internal_call(p, "__ivar_getOffset", args);
...
gb_internal lbProcedure *lb_create_main_procedure(lbModule *m, lbProcedure *startup_runtime, lbProcedure *cleanup_runtime) {
    ...
    if (has_args) { // initialize `internal.args__`
        ...
        lbAddr args = lb_addr(lb_find_internal_value(p->module, str_lit("args__")));
    ...
    if (build_context.command_kind == Command_test) {
        ...
        lbValue exit_runner = {};
        {
            AstPackage *pkg = get_internal_package(m->info);
