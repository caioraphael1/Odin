...
gb_internal void lb_build_range_string(lbProcedure *p, lbValue expr, Type *val_type,
    ...
    if (!is_reverse) {
        /*
            ...
            offset := 0
            for offset < len(str) {
                c, _w := __string_decode_rune(str[offset:])
                ...
                offset += _w
            }
        */
        ...
        /*
            ...
            offset := len(str)
            for offset > 0 {
                c, _w := __string_decode_last_rune(str[:offset])
                offset -= _w
                ...
            }
        */
    ...
    if (!is_reverse) {
        ...
        rune_and_len = lb_emit_internal_call(p, "__string_decode_rune", args);
        ...
        rune_and_len = lb_emit_internal_call(p, "__string_decode_last_rune", args);
        lbValue len  = lb_emit_struct_ev(p, rune_and_len, 1);
...
gb_internal void lb_build_range_string16(lbProcedure *p, lbValue expr, Type *val_type,
    ...
    if (!is_reverse) {
        /*
            ...
            offset := 0
            for offset < len(str) {
                c, _w := __string16_decode_rune(str[offset:])
                ...
                offset += _w
            }
        */
        ...
        /*
            ...
            offset := len(str)
            for offset > 0 {
                c, _w := __string16_decode_last_rune(str[:offset])
                offset -= _w
                ...
            }
        */
    ...
    if (!is_reverse) {
        ...
        rune_and_len = lb_emit_internal_call(p, "__string16_decode_rune", args);
        lbValue len  = lb_emit_struct_ev(p, rune_and_len, 1);
        lb_addr_store(p, offset_, lb_emit_arith(p, Token_Add, offset, len, t_int));
        ...
        rune_and_len = lb_emit_internal_call(p, "__string16_decode_last_rune", args);
        lbValue len  = lb_emit_struct_ev(p, rune_and_len, 1);
        lb_addr_store(p, offset_, lb_emit_arith(p, Token_Sub, offset, len, t_int));
...
gb_internal void lb_emit_defer_stmts(lbProcedure *p, lbDeferExitKind kind, lbBlock *block, TokenPos pos) {
    ...
    if (kind == lbDeferExit_Return) {
        for_array(i, p->asan_stack_locals) {
            ...
            lb_emit_internal_call(p, "__asan_unpoison_memory_region", args);
        }
    }
