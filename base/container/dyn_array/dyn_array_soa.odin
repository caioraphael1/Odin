...
make_soa_dynamic_array :: proc($T: typeid/#soa[dynamic]$E, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
    array.allocator = allocator
    reserve_soa(&array, 0, loc) or_return
    return array, nil
}

make_soa_dynamic_array_len :: proc($T: typeid/#soa[dynamic]$E, #any_int len: uint, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
    array.allocator = allocator
    resize_soa(&array, len, loc) or_return
    return array, nil
}

make_soa_dynamic_array_len_cap :: proc($T: typeid/#soa[dynamic]$E, #any_int len, cap: uint, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
    reserve_soa(&array, cap, loc) or_return
    resize_soa(&array, len, loc) or_return
    return array, nil
}


resize_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int len: uint, loc := #caller_location) -> mem.Allocator_Error {
    if array == nil {
        return nil
    }
    ...
    if len > footer.cap {
        reserve_soa(array, len, loc) or_return
    } else if size_of(E) > 0 && len > footer.len {
        ...
        for i in 0..<field_count {
            ...
            mem_zero(rawptr(uintptr(data) + uintptr(soa_offset) + uintptr(type.size * footer.len)), type.size * (len - footer.len))
...
non_zero_resize_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int len: uint, loc := #caller_location) -> mem.Allocator_Error {
    if array == nil {
        return nil
    }
    non_zero_reserve_soa(array, len, loc) or_return
    footer := raw_soa_footer(array)
    footer.len = len
    return nil
}

reserve_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int cap: uint, loc := #caller_location) -> mem.Allocator_Error {
    return _reserve_soa(array, cap, true, loc)
}

non_zero_reserve_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int cap: uint, loc := #caller_location) -> mem.Allocator_Error {
    return _reserve_soa(array, cap, false, loc)
}

_reserve_soa :: proc(array: ^$T/#soa[dynamic]$E, cap: uint, zero_memory: bool, loc := #caller_location) -> mem.Allocator_Error {
    if array == nil {
        return nil
    }
...
append_soa_elem :: proc(array: ^$T/#soa[dynamic]$E, #no_broadcast arg: E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    return _append_soa_elem(array, true, arg, loc)
}

non_zero_append_soa_elem :: proc(array: ^$T/#soa[dynamic]$E, #no_broadcast arg: E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    return _append_soa_elem(array, false, arg, loc)
}

_append_soa_elem :: proc(array: ^$T/#soa[dynamic]$E, zero_memory: bool, #no_broadcast arg: E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    if array == nil {
        return 0, nil
    }
...
append_soa_elems :: proc(array: ^$T/#soa[dynamic]$E, #no_broadcast args: ..E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    return _append_soa_elems(array, true, args=args, loc=loc)
}

non_zero_append_soa_elems :: proc(array: ^$T/#soa[dynamic]$E, #no_broadcast args: ..E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    return _append_soa_elems(array, false, args=args, loc=loc)
}


_append_soa_elems :: proc(array: ^$T/#soa[dynamic]$E, zero_memory: bool, #no_broadcast args: []E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    if array == nil {
        return
    }
...
append_nothing_soa :: proc(array: ^$T/#soa[dynamic]$E, loc := #caller_location) -> (n: uint, err: mem.Allocator_Error) {
    if array == nil {
        return 0, nil
    }
    prev_len := len(array)
...
inject_at_elem_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int index: uint, #no_broadcast arg: E, loc := #caller_location) -> (ok: bool, err: mem.Allocator_Error) #no_bounds_check {
...
inject_at_elems_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int index: uint, #no_broadcast args: ..E, loc := #caller_location) -> (ok: bool, err: mem.Allocator_Error) #no_bounds_check {
    when !ODIN_NO_BOUNDS_CHECK {
        internal.ensure(index >= 0, "Index must be positive.", loc)
    }
    if array == nil {
...
delete_soa_dynamic_array :: proc(array: $T/#soa[dynamic]$E, loc := #caller_location) -> mem.Allocator_Error {
    field_count :: len(E) when intrinsics.type_is_array(E) else intrinsics.type_struct_field_count(E)
    when field_count != 0 {
        array := array
        ptr := (^rawptr)(&array)^
...
// with the old value, and reducing the len of the dynamic array by 1.
//
// Note: This is an O(1) operation.
// Note: If you the elements to remain in their order, use `ordered_remove_soa`.
// Note: If the index is out of bounds, this procedure will panic.
unordered_remove_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int index: uint, loc := #caller_location) #no_bounds_check {
...
// Note: If the index is out of bounds, this procedure will panic.
ordered_remove_soa :: proc(array: ^$T/#soa[dynamic]$E, #any_int index: uint, loc := #caller_location) #no_bounds_check {
    bounds_check_error_loc(loc, index, len(array))
    if index+1 < len(array) {
        ti := type_info_of(typeid_of(T))
        ti = type_info_base(ti)
        ...
        for i in 0..<field_count {
            ...
            len := type.size*(len(array) - index - 1)
            mem_copy(rawptr(offset), rawptr(offset + uintptr(type.size)), len)
            data += size_of(rawptr)
        }
    }
    raw_soa_footer_dynamic_array(array).len -= 1
