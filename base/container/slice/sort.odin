...
cmp :: proc(a, b: $E) -> Ordering where intrinsics.type_is_ordered(E) {
    switch {
    case a < b:
        return .Less
    case a > b:
...
cmp_proc :: proc($E: typeid) -> (proc(E, E) -> Ordering) where intrinsics.type_is_ordered(E) {
...
sort_by_indices_allocate :: proc(data: $T/[]$E, indices: []uint, allocator: mem.Allocator) -> (sorted: T) {
...
_sort_by_indices :: proc(data, sorted: $T/[]$E, indices: []uint) {
    internal.assert(len(data) == len(indices))
...
sort_by_indices_overwrite :: proc(data: $T/[]$E, indices: []uint) {
    internal.assert(len(data) == len(indices))
...
sort_from_permutation_indices :: proc(data: $T/[]$E, indices: []uint) {
    internal.assert(len(data) == len(indices))
    if len(indices) <= 1 {
        return
    }
...
// This sort is not guaranteed to be stable
sort_with_indices :: proc(data: $T/[]$E, allocator: mem.Allocator) -> (indices: []uint) where intrinsics.type_is_ordered(E) {
    indices = slice.create([]uint, len(data), allocator)
    when size_of(E) != 0 {
        if n := len(data); n > 1 {
...
// sort_by sorts a slice with a given procedure to test whether two values are ordered "i < j"
// This sort is not guaranteed to be stable
sort_by_with_indices :: proc(data: $T/[]$E, less: proc(i, j: E) -> bool, allocator: mem.Allocator) -> (indices : []uint) {
    indices = slice.create([]uint, len(data), allocator)
    when size_of(E) != 0 {
        if n := len(data); n > 1 {
            for _, idx in indices {
...
sort_by_with_indices_with_data :: proc(data: $T/[]$E, less: proc(i, j: E, user_data: rawptr) -> bool, user_data: rawptr, allocator: mem.Allocator) -> (indices : []uint) {
    indices = slice.create([]uint, len(data), allocator)
    when size_of(E) != 0 {
        if n := len(data); n > 1 {
            for _, idx in indices {
...
*/
stable_sort :: proc(data: $T/[]$E) where intrinsics.type_is_ordered(E) {
    when size_of(E) != 0 {
        if n := len(data); n > 1 {
...
is_sorted :: proc(array: $T/[]$E) -> bool where intrinsics.type_is_ordered(E) {
...
reverse_sort :: proc(data: $T/[]$E) where intrinsics.type_is_ordered(E) {
...
sort_by_key :: proc(data: $T/[]$E, key: proc(E) -> $K) where intrinsics.type_is_ordered(K) {
    Context :: struct {
        key: proc(E) -> K,
    }
    ctx := &Context{key}
...
reverse_sort_by_key :: proc(data: $T/[]$E, key: proc(E) -> $K) where intrinsics.type_is_ordered(K) {
    Context :: struct {
        key: proc(E) -> K,
    }
    ctx := &Context{key}
...
is_sorted_by_key :: proc(array: $T/[]$E, key: proc(E) -> $K) -> bool where intrinsics.type_is_ordered(K) {
    for i := len(array)-1; i > 0; i -= 1 {
