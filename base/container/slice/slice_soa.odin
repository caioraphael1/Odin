...
create_soa_slice :: proc($T: typeid/#soa[]$E, #any_int length: uint, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
    return create_soa_aligned(T, length, align_of(E), allocator, loc)
}

create_soa_aligned :: proc($T: typeid/#soa[]$E, #any_int length, alignment: uint, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
    if length <= 0 {
        return
    }
