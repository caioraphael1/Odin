...
linear_search_proc :: proc(array: $A/[]$T, f: proc(T) -> bool) -> (index: uint, found: bool) {
    for x, i in array {
        if f(x) {
            return i, true
        }
...
Example:
    ...
    internal.assert(index == 1 && found == true)
*/
linear_search_reverse :: proc(array: $A/[]$T, key: T) -> (index: uint, found: bool) where intrinsics.type_is_comparable(T) {
...
*/
linear_search_reverse_proc :: proc(array: $A/[]$T, f: proc(T) -> bool) -> (index: uint, found: bool) {
    #reverse for x, i in array {
        if f(x) {
            return i, true
        }
...
If the value is found then the returned uint is the index of the matching element.
If there are multiple matches, then any one of the matches could be returned.

If the value is not found then the returned uint is the index where a matching
...
*/
binary_search :: proc(array: $A/[]$T, key: T) -> (index: uint, found: bool) where intrinsics.type_is_ordered(T) #no_bounds_check {
...
- found: bool
*/
binary_search_by :: proc(array: $A/[]$T, key: $K, f: proc(T, K) -> Ordering) -> (index: uint, found: bool) #no_bounds_check {
    n:     uint = len(array)
    left:  uint = 0
    right: uint = n
    for left < right {
