...
static_add :: proc(m: ^$H/Static_Handle_Map($N, $T, $Handle_Type), item: T) -> (handle: Handle_Type, ok: bool) {
...
static_get :: proc(m: ^$H/Static_Handle_Map($N, $T, $Handle_Type), h: Handle_Type) -> (^T, bool) {
...
static_remove :: proc(m: ^$H/Static_Handle_Map($N, $T, $Handle_Type), h: Handle_Type) -> bool {
    if h.idx <= 0 || u32(h.idx) >= m.used_len {
        return false
    }
...
static_is_valid :: proc(m: $H/Static_Handle_Map($N, $T, $Handle_Type), h: Handle_Type) -> bool {
    return h.idx > 0 && u32(h.idx) < m.used_len && m.items[h.idx].handle == h
}

// Returns the number of possibly valid items in the handle map.

static_len :: proc(m: $H/Static_Handle_Map($N, $T, $Handle_Type)) -> uint {
...
static_cap :: proc(m: $H/Static_Handle_Map($N, $T, $Handle_Type)) -> uint {
...
static_iterator_make :: proc(m: ^$H/Static_Handle_Map($N, $T, $Handle_Type)) -> Static_Handle_Map_Iterator(H) {
    return {m, 1}
}
...
static_iterate :: proc(it: ^$HI/Static_Handle_Map_Iterator($H/Static_Handle_Map($N, $T, $Handle_Type))) -> (val: ^T, h: Handle_Type, ok: bool) {
