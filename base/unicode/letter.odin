...
binary_search :: proc(c: i32, table: []i32, length, stride: int, loc := #caller_location) -> int #no_bounds_check {
    internal.bounds_check_error_loc(loc, length*stride-1, len(table))
    n := length
    t := 0
    for n > 1 {
        m := n / 2
...
normalized_east_asian_width :: proc(r: rune) -> uint #no_bounds_check {
    ...
    if p >= 0 && normalized_east_asian_width_ranges[p] <= c && c <= normalized_east_asian_width_ranges[p+1] {
        return cast(uint)normalized_east_asian_width_ranges[p+2]
    }
    return 1
}
