...
@(optional_results)
encode_string :: proc(d: []u16, s: string) -> uint {
    n: uint
    m := len(d)
    loop: for r in s {
        switch r {
...
@(optional_results)
decode :: proc(d: []rune, s: []u16) -> (n: uint) {
...
rune_from_string :: proc(s: string16) -> (r: rune, width: int) {
    r = rune(REPLACEMENT_CHAR)
    n := len(s)
    if n < 1 {
        return
...
string_rune_count :: proc(s: string16) -> (n: uint) {
    for i: uint = 0; i < len(s); i += 1 {
        c := s[i]
...
bytes_rune_count :: proc(s: []u16) -> (n: uint) {
    for i: uint = 0; i < len(s); i += 1 {
        c := s[i]
        if _surr1 <= c && c < _surr2 && i+1 < len(s) && 
            _surr2 <= s[i+1] && s[i+1] < _surr3 {
...
decode_to_utf8 :: proc(d: []u8, s: []u16) -> (n: uint) {
    for i: uint = 0; i < len(s); i += 1 {
        ...
        b, w := utf8.bytes_from_rune(rune(r))
