...
map_seed_from_map_data :: #force_inline proc(data: uintptr) -> uintptr {
...
map_desired_position :: #force_inline proc(m: Raw_Map, hash: Map_Hash) -> uintptr {
...
map_probe_distance :: #force_inline proc(m: Raw_Map, hash: Map_Hash, slot: uintptr) -> uintptr {
    // We do not use map_cap since we know the capacity will not be zero here.
    capacity := uintptr(1) << map_log2_cap(m)
...
map_insert_hash_dynamic :: #force_inline proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, h: Map_Hash, ik: uintptr, iv: uintptr) -> (result: uintptr) {
...
map_shrink_dynamic :: #force_no_inline proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, loc := #caller_location) -> (did_shrink: bool, err: Allocator_Error) {
...
__dynamic_map_get_key_and_value :: proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, h: Map_Hash, key: rawptr) -> (key_ptr, value_ptr: rawptr) {
    if m.len == 0 {
        return nil, nil
    }
    pos := map_desired_position(m^, h)
...
__dynamic_map_set_without_hash :: proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, key, value: rawptr, loc := #caller_location) -> rawptr {
...
__dynamic_map_set_extra_without_hash :: proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, key, value: rawptr, loc := #caller_location) -> (prev_key_ptr, value_ptr: rawptr) {
    return __dynamic_map_set_extra(m, info, info.key_hasher(key, map_seed(m^)), key, value, loc)
}

__dynamic_map_set_extra :: proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, hash: Map_Hash, key, value: rawptr, loc := #caller_location) -> (prev_key_ptr, value_ptr: rawptr) {
...
__dynamic_map_entry :: proc(#no_alias m: ^Raw_Map, #no_alias info: ^Map_Info, key: rawptr, zero: rawptr, loc := #caller_location) -> (key_ptr: rawptr, value_ptr: rawptr, just_inserted: bool, err: Allocator_Error) {
    hash := info.key_hasher(key, map_seed(m^))
...
default_hasher :: #force_inline proc(data: rawptr, seed: uintptr, N: int) -> uintptr {
...
default_hasher_string :: proc(data: rawptr, seed: uintptr) -> uintptr {
    str := (^[]byte)(data)
    return default_hasher(raw_data(str^), seed, len(str))
}
default_hasher_cstring :: proc(data: rawptr, seed: uintptr) -> uintptr {
    h := u64(seed) + INITIAL_HASH_SEED
...
default_hasher_f64 :: proc(f: f64, seed: uintptr) -> uintptr {
    f := f
    buf: [size_of(f)]u8
    if f == 0 {
...
default_hasher_complex128 :: proc(x, y: f64, seed: uintptr) -> uintptr {
    seed := seed
...
default_hasher_quaternion256 :: proc(x, y, z, w: f64, seed: uintptr) -> uintptr {
    seed := seed
    seed = default_hasher_f64(x, seed)
    seed = default_hasher_f64(y, seed)
