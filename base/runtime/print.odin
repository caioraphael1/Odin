...
when !ODIN_NO_RTTI {
    print_any_single :: #force_no_inline proc(arg: any) {
        x := arg
        if x.data == nil {
            print_string("nil")
            return
...
print_uint    :: proc(x: uint)    { print_u64(u64(x)) }
print_uintptr :: proc(x: uintptr) { print_u64(u64(x)) }
print_int     :: proc(x: int)     { print_i64(i64(x)) }
...
print_typeid :: #force_no_inline proc(id: typeid) {
    when ODIN_NO_RTTI {
        if id == nil {
            print_string("nil")
        } else {
...
print_type :: #force_no_inline proc(ti: ^Type_Info) {
    if ti == nil {
        print_string("nil")
        return
    }
