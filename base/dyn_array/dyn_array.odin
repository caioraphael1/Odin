...
create_len :: proc($T: typeid/[dynamic]$E, #any_int len: int, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
...
// Unlike `new`, `make`'s return value is the same as the type of its argument, not a pointer to it.
create_len_cap :: proc($T: typeid/[dynamic]$E, #any_int len: int, #any_int cap: int, allocator: mem.Allocator, loc := #caller_location) -> (array: T, err: mem.Allocator_Error) {
...
_append :: #force_no_inline proc(array: ^Raw_Dynamic_Array, size_of_elem, align_of_elem: int, arg_ptr: rawptr, should_zero: bool, loc := #caller_location) -> (err: mem.Allocator_Error) {
    if array == nil {
        return
    }
...
_append_many :: #force_no_inline proc(array: ^Raw_Dynamic_Array, size_of_elem, align_of_elem: int, should_zero: bool, loc := #caller_location, args: rawptr, arg_len: int) -> (err: mem.Allocator_Error) {
    if array == nil {
        return nil
    }
...
assign_at :: proc(array: ^$T/[dynamic]$E, #any_int index: int, arg: E, loc := #caller_location) -> (ok: bool, err: mem.Allocator_Error) #no_bounds_check {
    if index < len(array) {
        array[index] = arg
        ok = true
    } else {
...
assign_many_at :: proc(array: ^$T/[dynamic]$E, #any_int index: int, #no_broadcast args: ..E, loc := #caller_location) -> (ok: bool, err: mem.Allocator_Error) #no_bounds_check {
...
assign_string_at :: proc(array: ^$T/[dynamic]$E/u8, #any_int index: int, arg: string, loc := #caller_location) -> (ok: bool, err: mem.Allocator_Error) #no_bounds_check {
...
reserve :: proc(array: ^$T/[dynamic]$E, #any_int capacity: int, loc := #caller_location) -> mem.Allocator_Error {
    return _reserve((^Raw_Dynamic_Array)(array), size_of(E), align_of(E), capacity, true, loc)
}

// `reserve` will try to reserve memory of a passed dynamic array or map to the requested element count (setting the `cap`).
// When a memory mem.resize allocation is required, the memory will be asked to be zeroed (i.e. it calls `mem.resize`).
_reserve :: #force_no_inline proc(a: ^Raw_Dynamic_Array, size_of_elem, align_of_elem: int, capacity: int, should_zero: bool, loc := #caller_location) -> mem.Allocator_Error {
    if a == nil {
        return nil
    }
...
reserve_non_zero :: proc(array: ^$T/[dynamic]$E, #any_int capacity: int, loc := #caller_location) -> mem.Allocator_Error {
...
// When a memory mem.resize allocation is required, the memory will be asked to be zeroed (i.e. it calls `mem.resize`).
resize :: proc(array: ^$T/[dynamic]$E, #any_int length: int, loc := #caller_location) -> mem.Allocator_Error {
...
// When a memory mem.resize allocation is required, the memory will be asked to not be zeroed (i.e. it calls `mem.resize_non_zero`).
resize_non_zero :: proc(array: ^$T/[dynamic]$E, #any_int length: int, loc := #caller_location) -> mem.Allocator_Error {
    return _resize((^Raw_Dynamic_Array)(array), size_of(E), align_of(E), length, false, loc=loc)
}

_resize :: #force_no_inline proc(a: ^Raw_Dynamic_Array, size_of_elem, align_of_elem: int, length: int, should_zero: bool, loc := #caller_location) -> mem.Allocator_Error {
    // Invalid pointer
    if a == nil {
        return nil
    }
...
shrink :: proc(array: ^$T/[dynamic]$E, #any_int new_cap := -1, loc := #caller_location) -> (did_shrink: bool, err: mem.Allocator_Error) {
    return _shrink((^Raw_Dynamic_Array)(array), size_of(E), align_of(E), new_cap, loc)
}

_shrink :: proc(a: ^Raw_Dynamic_Array, size_of_elem, align_of_elem: int, new_cap := -1, loc := #caller_location) -> (did_shrink: bool, err: mem.Allocator_Error) {
    // Invalid pointer
    if a == nil {
        return
    }
...
@(disabled=ODIN_NO_BOUNDS_CHECK)
create_error_loc :: #force_inline proc(loc := #caller_location, len, cap: int) {
    if 0 <= len && len <= cap {
        return
    }
    @(cold, no_instrumentation)
    handle_error :: proc(loc: internal.Source_Code_Location, len, cap: int)  -> ! {
