...
generic_ftoa :: proc(buf: []u8, val: f64, fmt: u8, precision, bit_size: uint, shortest: bool) -> []u8 {
    ...
    if shortest {
        ...
        case 'f', 'F': prec = uint(max(int(digs.count) - digs.decimal_point, 0))
        ...
        case 'f', 'F':
            decimal.round(d, uint(d.decimal_point) + prec)
        case 'g', 'G':
            if prec == 0 {
                prec = 1
            }
...
format_digits :: proc(buf: []u8, shortest: bool, neg: bool, digs: Decimal_Slice, precision: uint, fmt: u8) -> []u8 {
    ...
    case 'f', 'F':
        ...
        if digs.decimal_point > 0 {
            m := min(int(digs.count), digs.decimal_point)
    ...
    case 'g', 'G':
        eprec := prec
        if eprec > digs.count && int(digs.count) >= digs.decimal_point {
            eprec = digs.count
        }
        ...
        if int(prec) > digs.decimal_point {
            prec = digs.count
        }

        return format_digits(buf, shortest, neg, digs, uint(max(int(prec) - digs.decimal_point, 0)), 'f')
...
round_shortest :: proc(d: ^decimal.Decimal, mant: u64, exp: int, flt: ^Float_Info) {
     ...
     */
    minexp := flt.bias+1
    if exp > minexp && 332*(d.decimal_point - int(d.count)) >= 100*(exp - int(flt.mantbits)) {
