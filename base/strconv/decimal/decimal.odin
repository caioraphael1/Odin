...
decimal_to_string :: proc(buf: []byte, a: ^Decimal) -> string {
    ...
    n := 10 + int(a.count) + abs(a.decimal_point)

    // TODO(bill): make this work with a buffer that's not big enough
    internal.assert(int(len(buf)) >= n)
    b := buf[0:n]
...
shift_left :: proc(a: ^Decimal, k: uint) #no_bounds_check {
    ...
    read_index  := int(a.count)
    write_index := int(a.count) + delta
    ...
    a.count = uint(clamp(int(a.count) + delta, 0, len(a.digits)))
    trim(a)
}
/*
...
*/
can_round_up :: proc(a: ^Decimal, nd: uint) -> bool {
    if nd >= a.count { return false  }
...
*/
round :: proc(a: ^Decimal, nd: uint) {
    if nd >= a.count { return }
...
*/
round_up :: proc(a: ^Decimal, nd: uint) {
    if nd >= a.count { return }

    for i := int(nd) - 1; i >= 0; i -= 1 {
        if c := a.digits[i]; c < '9' {
            a.digits[i] += 1
            a.count = uint(i) + 1
            return
...
*/
round_down :: proc(a: ^Decimal, nd: uint) {
    if nd >= a.count { return }
    a.count = nd
    trim(a)
}
...
rounded_integer :: proc(a: ^Decimal) -> u64 {
    ...
    i: int = 0
    n: u64 = 0
    m := min(a.decimal_point, int(a.count))
    for ; i < m; i += 1 {
    ...
    if can_round_up(a, uint(a.decimal_point)) {
        n += 1
    }
    return n
}
