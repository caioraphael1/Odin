...
*/
array_init :: proc(x: ^$X/Array($T, $SHIFT), allocator: mem.Allocator) {
...
array_get :: proc(x: ^$X/Array($T, $SHIFT), #any_int index: int, loc := #caller_location) -> (val: T) #no_bounds_check {
    internal.bounds_check_error_loc(loc, index, x.len)
...
// No bounds checking
array_get_ptr_unsafe :: proc(x: ^$X/Array($T, $SHIFT), #any_int index: int) -> (val: ^T) #no_bounds_check {
    chunk_idx, elem_idx, _ := _meta_get(SHIFT, uint(index))
    return &x.chunks[chunk_idx][elem_idx]
}
