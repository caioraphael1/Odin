...
nil_allocator :: proc() -> mem.Allocator {
...
arena_alloc_bytes_non_zeroed :: proc(
    ...
    result := slice_of_bytes(ptr, size)
...
arena_allocator_proc :: proc(
    ...
    case .Resize:
        return default_resize_bytes_align(slice_of_bytes(old_memory, old_size), size, alignment, arena_allocator(arena), loc)
    case .Resize_Non_Zeroed:
        return default_resize_bytes_align_non_zeroed(slice_of_bytes(old_memory, old_size), size, alignment, arena_allocator(arena), loc)
    case .Query_Features:
        set := (^Allocator_Mode_Set)(old_memory)
        if set != nil {
...
scratch_init :: proc(s: ^Scratch, size: int, backup_allocator: mem.Allocator) -> mem.Allocator_Error {
    s.data = internal.slice_create_aligned([]byte, size, 2*align_of(rawptr), backup_allocator) or_return
    s.curr_offset = 0
...
scratch_alloc_bytes_non_zeroed :: proc(
    ...
    if aligned_size <= len(s.data) {
        ...
        ptr, err := internal.mem_alloc_non_zeroed(size, alignment, a, loc)
        if err != nil {
            return ptr, err
        }
...
scratch_resize :: proc(
    ...
    bytes, err := scratch_resize_bytes(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    return raw_data(bytes), err
}
...
scratch_resize_non_zeroed :: proc(
    ...
    bytes, err := scratch_resize_bytes_non_zeroed(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    return raw_data(bytes), err
}
...
scratch_resize_bytes_non_zeroed :: proc(
    ...
    if s.prev_allocation == old_memory && is_aligned(old_memory, alignment) && old_ptr+uintptr(size) < end {
        ...
        result := slice_of_bytes(old_memory, size)
        // sanitizer.address_unpoison(result)
        return result, nil
    }
    ...
    internal.slice_copy(data, slice_of_bytes(old_memory, old_size))
...
scratch_allocator_proc :: proc(
    ...
    case .Resize:
        return scratch_resize_bytes(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Resize_Non_Zeroed:
        return scratch_resize_bytes_non_zeroed(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Query_Features:
        set := (^Allocator_Mode_Set)(old_memory)
        if set != nil {
...
stack_alloc_bytes_non_zeroed :: proc(
    ...
    result := slice_of_bytes(rawptr(next_addr), size)
    // ensure_poisoned(result)
    // sanitizer.address_unpoison(result)
    return result, nil
}
...
stack_resize :: proc(
    ...
    bytes, err := stack_resize_bytes(s, slice_of_bytes(old_memory, old_size), size, alignment)
    return raw_data(bytes), err
}
...
stack_resize_non_zeroed :: proc(
    ...
    bytes, err := stack_resize_bytes_non_zeroed(s, slice_of_bytes(old_memory, old_size), size, alignment)
    return raw_data(bytes), err
}
...
stack_resize_bytes_non_zeroed :: proc(
    ...
    if old_size == size {
        return slice_of_bytes(old_memory, size), nil
    }
    header := (^Stack_Allocation_Header)(curr_addr - size_of(Stack_Allocation_Header))
    old_offset := int(curr_addr - uintptr(header.padding) - uintptr(raw_data(s.data)))
...
stack_allocator_proc :: proc(
    ...
    case .Resize:
        return stack_resize_bytes(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Resize_Non_Zeroed:
        return stack_resize_bytes_non_zeroed(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Query_Features:
        set := (^Allocator_Mode_Set)(old_memory)
        if set != nil {
            set^ = {.Alloc, .Alloc_Non_Zeroed, .Free, .Free_All, .Resize, .Resize_Non_Zeroed, .Query_Features}
...
small_stack_resize :: proc(
    ...
    bytes, err := small_stack_resize_bytes(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    return raw_data(bytes), err
}
...
small_stack_resize_non_zeroed :: proc(
    ...
    bytes, err := small_stack_resize_bytes_non_zeroed(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    return raw_data(bytes), err
}
...
small_stack_allocator_proc :: proc(
    ...
    case .Resize:
        return small_stack_resize_bytes(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Resize_Non_Zeroed:
        return small_stack_resize_bytes_non_zeroed(s, slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Query_Features:
        set := (^Allocator_Mode_Set)(old_memory)
        if set != nil {
            set^ = {.Alloc, .Alloc_Non_Zeroed, .Free, .Free_All, .Resize, .Resize_Non_Zeroed, .Query_Features}
...
dynamic_arena_alloc :: proc(a: ^Dynamic_Arena, size: int, loc := #caller_location) -> (rawptr, mem.Allocator_Error) {
...
dynamic_arena_alloc_bytes :: proc(a: ^Dynamic_Arena, size: int, loc := #caller_location) -> ([]byte, mem.Allocator_Error) {
...
dynamic_arena_alloc_non_zeroed :: proc(a: ^Dynamic_Arena, size: int, loc := #caller_location) -> (rawptr, mem.Allocator_Error) {
...
dynamic_arena_alloc_bytes_non_zeroed :: proc(a: ^Dynamic_Arena, size: int, loc := #caller_location) -> ([]byte, mem.Allocator_Error) {
    if size >= a.out_band_size {
        ...
        memory, err := internal.mem_alloc_non_zeroed(size, a.alignment, a.out_band_allocations.allocator, loc)
        if memory != nil {
...
dynamic_arena_resize :: proc(
    ...
    bytes, err := dynamic_arena_resize_bytes(a, slice_of_bytes(old_memory, old_size), size, loc)
    return raw_data(bytes), err
}
...
dynamic_arena_resize_non_zeroed :: proc(
    ...
    bytes, err := dynamic_arena_resize_bytes_non_zeroed(a, slice_of_bytes(old_memory, old_size), size, loc)
    return raw_data(bytes), err
}
...
dynamic_arena_allocator_proc :: proc(
    ...
    case .Resize:
        return dynamic_arena_resize_bytes(arena, slice_of_bytes(old_memory, old_size), size, loc)
    case .Resize_Non_Zeroed:
        return dynamic_arena_resize_bytes_non_zeroed(arena, slice_of_bytes(old_memory, old_size), size, loc)
    case .Query_Features:
        set := (^Allocator_Mode_Set)(old_memory)
        if set != nil {
    ...
    case .Query_Info:
        ...
        if info != nil && info.pointer != nil {
            ...
            return slice_of_bytes(info, size_of(info^)), nil
        }
        return nil, nil
    }
    return nil, nil
...
buddy_allocator_proc :: proc(
    ...
    case .Resize:
        return default_resize_bytes_align(slice_of_bytes(old_memory, old_size), size, alignment, buddy_allocator(b), loc)
    case .Resize_Non_Zeroed:
        return default_resize_bytes_align_non_zeroed(slice_of_bytes(old_memory, old_size), size, alignment, buddy_allocator(b), loc)
    case .Free:
