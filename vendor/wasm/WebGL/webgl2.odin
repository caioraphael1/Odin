...
GetActiveUniformBlockNameAlloc :: proc(program: Program, uniformBlockIndex: i32, allocator: mem.Allocator, loc := #caller_location) -> string {
    ...
    if n > 0 {
        buf := slice_create([]byte, n, allocator, loc)
        _GetActiveUniformBlockName(program, uniformBlockIndex, buf, &n)
        assert(n == len(buf))
        return string(buf[:n])
    }
...
Uniform1uiv :: proc(location: i32, v: u32) {
    Uniform1ui(location, v)
}
Uniform2uiv :: proc(location: i32, v: glm.uvec2) {
    Uniform2ui(location, v.x, v.y)
}
Uniform3uiv :: proc(location: i32, v: glm.uvec3) {
    Uniform3ui(location, v.x, v.y, v.z)
}
Uniform4uiv :: proc(location: i32, v: glm.uvec4) {
    Uniform4ui(location, v.x, v.y, v.z, v.w)
}

UniformMatrix3x2fv :: proc(location: i32, m: glm.mat3x2) {
    foreign webgl2 {
        @(link_name="UniformMatrix3x2fv")
        _UniformMatrix3x2fv :: proc(location: i32, addr: [^]f32) ---
    }
    array := intrinsics.matrix_flatten(m)
    _UniformMatrix3x2fv(location, &array[0])
}
UniformMatrix4x2fv :: proc(location: i32, m: glm.mat4x2) {
    foreign webgl2 {
        @(link_name="UniformMatrix4x2fv")
        _UniformMatrix4x2fv :: proc(location: i32, addr: [^]f32) ---
    }
    array := intrinsics.matrix_flatten(m)
    _UniformMatrix4x2fv(location, &array[0])
}
UniformMatrix2x3fv :: proc(location: i32, m: glm.mat2x3) {
    foreign webgl2 {
        @(link_name="UniformMatrix2x3fv")
        _UniformMatrix2x3fv :: proc(location: i32, addr: [^]f32) ---
    }
    array := intrinsics.matrix_flatten(m)
    _UniformMatrix2x3fv(location, &array[0])
}
UniformMatrix4x3fv :: proc(location: i32, m: glm.mat4x3) {
    foreign webgl2 {
        @(link_name="UniformMatrix4x3fv")
        _UniformMatrix4x3fv :: proc(location: i32, addr: [^]f32) ---
    }
    array := intrinsics.matrix_flatten(m)
    _UniformMatrix4x3fv(location, &array[0])
}
UniformMatrix2x4fv :: proc(location: i32, m: glm.mat2x4) {
    foreign webgl2 {
        @(link_name="UniformMatrix2x4fv")
        _UniformMatrix2x4fv :: proc(location: i32, addr: [^]f32) ---
    }
    array := intrinsics.matrix_flatten(m)
    _UniformMatrix2x4fv(location, &array[0])
}
UniformMatrix3x4fv :: proc(location: i32, m: glm.mat3x4) {
    foreign webgl2 {
        @(link_name="UniformMatrix3x4fv")
        _UniformMatrix3x4fv :: proc(location: i32, addr: [^]f32) ---
    }
    array := intrinsics.matrix_flatten(m)
    _UniformMatrix3x4fv(location, &array[0])
}

VertexAttribI4iv :: proc(index: i32, v: glm.ivec4) {
    VertexAttribI4i(index, v.x, v.y, v.z, v.w)
}
VertexAttribI4uiv :: proc(index: i32, v: glm.uvec4) {
