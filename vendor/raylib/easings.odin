...
EaseSineIn    :: proc(t, b, c, d: f32) -> f32 { return (-c*math.cos_f32(t/d*(PI/2.0)) + c + b) }
EaseSineOut   :: proc(t, b, c, d: f32) -> f32 { return (c*math.sin_f32(t/d*(PI/2.0)) + b) }
EaseSineInOut :: proc(t, b, c, d: f32) -> f32 { return (-c/2.0*(math.cos_f32(PI*t/d) - 1.0) + b) }
...
EaseExpoIn :: proc(t, b, c, d: f32) -> f32 { 
    return (t == 0.0) ? b : (c*math.pow_f32(2.0, 10.0*(t/d - 1.0)) + b)
}
EaseExpoOut :: proc(t, b, c, d: f32) -> f32 { 
    return (t == d) ? (b + c) : (c*(-math.pow_f32(2.0, -10.0*t/d) + 1.0) + b)
}
EaseExpoInOut :: proc(t, b, c, d: f32) -> f32 {
    ...
    t /= d/2.0
    if t < 1.0 {
        return c/2.0*math.pow_f32(2.0, 10.0*(t - 1.0)) + b
    }

    return c/2.0*(-math.pow_f32(2.0, -10.0*(t - 1.0)) + 2.0) + b
}
...
EaseElasticIn :: proc(t, b, c, d: f32) -> f32 {
    ...
    s := p/4.0
    t -= 1
    postFix := a*math.pow_f32(2.0, 10.0*t)

    return -(postFix*math.sin_f32((t*d-s)*(2.0*PI)/p )) + b
}
...
EaseElasticOut :: proc(t, b, c, d: f32) -> f32 {
    ...
    return a*math.pow_f32(2.0,-10.0*t)*math.sin_f32((t*d-s)*(2.0*PI)/p) + c + b
}
...
EaseElasticInOut :: proc(t, b, c, d: f32) -> f32 {
    ...
    if t < 1.0 {
        postFix := a*math.pow_f32(2.0, 10.0*t)
        return -0.5*(postFix*math.sin_f32((t*d-s)*(2.0*PI)/p)) + b
    }

    postFix := a*math.pow_f32(2.0, -10.0*t)
    return (postFix*math.sin_f32((t*d-s)*(2.0*PI)/p)*0.5 + c + b)
}
