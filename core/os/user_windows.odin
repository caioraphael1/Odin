...
_local_appdata :: proc(allocator: mem.Allocator) -> (dir: string, err: Error) {
    guid := win32.FOLDERID_LocalAppData
    return _get_known_folder_path(&guid, allocator)
}

_local_appdata_or_roaming :: proc(allocator: mem.Allocator, roaming: bool) -> (dir: string, err: Error) {
    guid := win32.FOLDERID_LocalAppData
    if roaming {
