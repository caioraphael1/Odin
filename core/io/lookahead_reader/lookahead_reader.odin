...
lookahead_reader_init :: proc(lr: ^Lookahead_Reader, r: io.Reader, buf: []u8) -> ^Lookahead_Reader {
    lr.r = r
    lr.buf = buf
    lr.n = 0
    return lr
...
lookahead_reader_peek :: proc(lr: ^Lookahead_Reader, n: uint) -> ([]u8, io.Error) {
    if n > len(lr.buf) {
...
lookahead_reader_consume :: proc(lr: ^Lookahead_Reader, n: int) -> io.Error {
    ...
    slice.copy(lr.buf, lr.buf[n:lr.n])
    lr.n -= n
    return nil
}
