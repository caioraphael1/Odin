...
writer_init :: proc(b: ^Writer, wr: io.Writer, size: uint = DEFAULT_BUF_SIZE, allocator: mem.Allocator) {
    size := size
...
writer_size :: proc(b: ^Writer) -> uint {
    return len(b.buf)
}
...
writer_flush :: proc(b: ^Writer) -> io.Error {
    ...
    if err != nil {
        if n > 0 && n < b.n {
            slice.copy(b.buf[:b.n-n], b.buf[n : b.n])
        }
        b.n -= n
        b.err = err
        return err
