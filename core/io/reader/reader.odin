...
Reader :: struct {
    ...
    rd:             io.Reader, // reader
    r, w:           uint, // read and write positions for buf
...
reader_init :: proc(b: ^Reader, rd: io.Reader, size: uint = DEFAULT_BUF_SIZE, allocator: mem.Allocator, loc := #caller_location) {
    size := size
...
reader_size :: proc(b: ^Reader) -> uint {
    return len(b.buf)
}
...
reader_buffered :: proc(b: ^Reader) -> uint {
    return b.w - b.r
}

// reader_discard skips the next n bytes, and returns the number of bytes that were discarded
reader_discard :: proc(b: ^Reader, n: uint) -> (discarded: uint, err: io.Error) {
    if n < 0 {
