...
marshal_to_writer :: proc(w: io.Writer, v: any, opt: ^Marshal_Options) -> (err: Marshal_Error) {
    ...
    case reflect.Type_Info_Integer:
        ...
        if opt.write_uint_as_hex && (opt.spec == .JSON5 || opt.spec == .MJSON) {
            ...
            case u8, u16, u32, u64, u128:
                s = strconv.write_bits_128(buf[:], u, 16, info.signed, 8*uint(ti.size), "0123456789abcdef", { .Prefix })

            case:
                s = strconv.write_bits_128(buf[:], u, 10, info.signed, 8*uint(ti.size), "0123456789", nil)
            }
        } else {
            s = strconv.write_bits_128(buf[:], u, 10, info.signed, 8*uint(ti.size), "0123456789", nil)
        }
    ...
    case reflect.Type_Info_Enumerated_Array:
        ...
        opt_write_start(w, opt, '{') or_return
        for i in 0..<info.count {
            value := cast(reflect.Type_Info_Enum_Value)i
            ...
            if !opt.sort_maps_by_key {
                ...
                for bucket_index in 0..<map_cap {
                    ...
                    // check for string type
                    {
                        ...
                        kti := reflect.type_info_base(type_info_of(kv.id))
    ...
    case reflect.Type_Info_Struct:
        ...
        marshal_struct_fields :: proc(w: io.Writer, v: any, opt: ^Marshal_Options) -> (err: Marshal_Error) {
            ti := reflect.type_info_base(type_info_of(v.id))
            info := ti.variant.(reflect.Type_Info_Struct)
