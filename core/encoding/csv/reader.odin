...
reader_init :: proc(reader: ^Reader, r: io.Reader, buffer_allocator: mem.Allocator) {
    switch reader.comma {
    case '\x00', '\n', '\r', 0xfffd:
        reader.comma = ','
    }

    _ = dyn_array.reserve(&reader.record_buffer, DEFAULT_RECORD_BUFFER_CAPACITY)
...
reader_init_with_string :: proc(reader: ^Reader, s: string, buffer_allocator: mem.Allocator) {
...
reader_destroy :: proc(r: ^Reader) {
    _ = dyn_array.delete(r.raw_buffer)
...
/*
    ...
    you won't need to _ = slice.delete the record or its fields.
*/
...
read :: proc(r: ^Reader, allocator: mem.Allocator) -> (record: []string, err: Error) {
    if r.reuse_record {
        ...
        _ = dyn_array.resize(&r.last_record, len(record))
        slice.copy(r.last_record[:], record)
    } else {
...
read_all :: proc(r: ^Reader, allocator: mem.Allocator) -> ([][]string, Error) {
    ...
    for {
        ...
        if rerr != nil {
            ...
            if record != nil {
                _ = dyn_array.append(&records, record)
            }
            return records[:], rerr
        }
        _ = dyn_array.append(&records, record)
...
read_from_string :: proc(input: string, record_allocator: mem.Allocator, buffer_allocator: mem.Allocator) -> (record: []string, n: int, err: Error) {
    ir: strings.Reader
...
read_all_from_string :: proc(input: string, records_allocator: mem.Allocator, buffer_allocator: mem.Allocator) -> ([][]string, Error) {
    ir: strings.Reader
    strings.reader_init(&ir, input)
    input_reader, _ := io.to_reader(strings.reader_to_stream(&ir))
...
_read_record :: proc(r: ^Reader, dst: ^dyn_array.Dyn_Array(string), allocator: mem.Allocator) -> ([]string, Error) {
    ...
    dyn_array.clear(&r.record_buffer)
    dyn_array.clear(&r.field_indices)
    ...
    parse_field: for {
        ...
        if len(line) == 0 || line[0] != '"' {
            ...
            _ = dyn_array.append_many(&r.record_buffer, ..field)
            _ = dyn_array.append(&r.field_indices, len(r.record_buffer))
            if i >= 0 {
            ...
            for {
                ...
                switch {
                case i >= 0:
                    _ = dyn_array.append_many(&r.record_buffer, ..line[:i])
                    line = line[i+quote_len:]
                    switch ch := next_rune(line); {
                    case ch == '"': // dyn_array.append quote
                ...
                case len(line) > 0:
                    _ = dyn_array.append_many(&r.record_buffer, ..line)
                    if err_read != nil {
                        break parse_field
                    }
                    line, err_read = read_line(r)
