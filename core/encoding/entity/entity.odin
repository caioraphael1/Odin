...
decode_xml :: proc(input: string, options := XML_Decode_Options{}, allocator: mem.Allocator) -> (decoded: string, err: Error) {
    l := len(input)
    if l == 0 { return "", .None }
    ...
    #no_bounds_check {
        if t.read_offset < len(t.src) {
            ...
            case t.r >= utf8.RUNE_SELF:
                t.r, t.w = utf8.rune_from_string(t.src[t.read_offset:])
...
escape_html :: proc(s: string, allocator: mem.Allocator, loc := #caller_location) -> (output: string, was_allocation: bool) {
    /*
        & -> &amp;
...
unescape_entity :: proc(s: string) -> (b: [8]u8, w: int, j: int) {
    ...
    if r2, _, ok := named_xml_entity_to_rune(entity_name); ok {
        b1, w1 := utf8.bytes_from_rune(r2[0])
        w += copy(b[w:], b1[:w1])
        if r2[1] != 0 {
            b2, w2 := utf8.bytes_from_rune(r2[1])
            w += copy(b[w:], b2[:w2])
        }
        return
    }
