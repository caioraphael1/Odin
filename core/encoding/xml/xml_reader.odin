...
Document :: struct {
    elements:      dyn_array.Dyn_Array(Element) `fmt:"v,element_count"`,
    ...
    comments: dyn_array.Dyn_Array(string)        `fmt:"-"`,
    ...
    input:           []u8            `fmt:"-"`,
    strings_to_free: dyn_array.Dyn_Array(string) `fmt:"-"`,
}

Element :: struct {
    ident:   string,
    value:   dyn_array.Dyn_Array(Value),
    attribs: Attributes,
...
Attributes :: dyn_array.Dyn_Array(Attribute)
...
destroy :: proc(doc: ^Document, allocator: mem.Allocator) {
    if doc == nil { return }
