...
destroy :: proc(control: ^Allocator) {
    ...
    if control.pool.allocator.procedure != nil {
        _ = internal.slice.delete(control.pool.data, control.pool.allocator)
    }
    ...
    for p := control.pool.next; p != nil; {
        ...
        _ = internal.slice.delete(p.data, p.allocator)
...
allocator_proc :: proc(allocator_data: rawptr, mode: mem.Allocator_Mode,
    ...
    switch mode {
    case .Alloc:
        return mem.alloc(control, uint(size), uint(alignment))
    case .Alloc_Non_Zeroed:
        return mem.alloc_non_zeroed(control, uint(size), uint(alignment))
