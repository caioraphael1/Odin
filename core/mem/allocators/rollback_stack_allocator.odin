...
*/
@(private="file", no_sanitize_address)
rb_free :: proc(stack: ^Rollback_Stack, ptr: rawptr) -> mem.Allocator_Error {
...
rb_resize :: proc(
    ...
    bytes, err := rb_resize_bytes_non_zeroed(stack, mem.slice_of_bytes(old_ptr, old_size), size, alignment, loc)
    if bytes != nil {
        if old_ptr == nil {
            mem.zero_slice(bytes)
...
rb_resize_bytes :: proc(
    ...
    if bytes != nil {
        if old_memory == nil {
            ...
            mem.zero_slice(bytes[len(old_memory):])
        }
    }
    return bytes, err
}
...
@(private="file", no_sanitize_address)
rb_make_block :: proc(size: int, allocator: mem.Allocator) -> (block: ^Rollback_Stack_Block, err: mem.Allocator_Error) {
...
The Rollback Stack mem.Allocator was designed for the test runner to be fast,
able to grow, and respect the Tracking mem.Allocator's requirement for
...
*/
@(no_sanitize_address)
rollback_stack_allocator :: proc(stack: ^Rollback_Stack) -> mem.Allocator {
...
rollback_stack_allocator_proc :: proc(
    ...
    case .Free_All:
        ...
        return nil, nil
    case .Resize:
        return rb_resize_bytes(stack, mem.slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Resize_Non_Zeroed:
        return rb_resize_bytes_non_zeroed(stack, mem.slice_of_bytes(old_memory, old_size), size, alignment, loc)
    case .Query_Features:
