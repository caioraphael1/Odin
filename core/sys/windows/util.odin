...
GET_X_LPARAM :: #force_inline proc(lp: LPARAM) -> c_int {
...
GET_Y_LPARAM :: #force_inline proc(lp: LPARAM) -> c_int {
...
MAKELCID :: #force_inline proc(lgid, srtid: WORD) -> LCID {
    return (DWORD(WORD(srtid)) << 16) | DWORD(WORD(lgid))
}

// https://learn.microsoft.com/en-us/windows/win32/api/winnt/nf-winnt-makelangid
MAKELANGID :: #force_inline proc(p, s: WORD) -> DWORD {
    return DWORD(WORD(s)) << 10 | DWORD(WORD(p))
}

LANGIDFROMLCID :: #force_inline proc(lcid: LCID) -> LANGID {
    return LANGID(lcid)
}

utf8_to_utf16_alloc :: proc(s: string, allocator: mem.Allocator) -> []u16 {
    if len(s) < 1 {
        return nil
    }
...
utf8_to_wstring_alloc :: proc(s: string, allocator: mem.Allocator) -> wstring {
...
*/
utf16_to_utf8_alloc :: proc(s: []u16, allocator: mem.Allocator) -> (res: string, err: mem.Allocator_Error) {
    if len(s) == 0 {
        return "", nil
    }
...
run_as_user :: proc(username, password, application, commandline: string, pi: ^PROCESS_INFORMATION, wait := true) -> (ok: bool) {
    ...
    */
    ...
    si := STARTUPINFOW{}
    si.cb = DWORD(size_of(STARTUPINFOW))
    pi := pi
