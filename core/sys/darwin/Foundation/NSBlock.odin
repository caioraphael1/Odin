...
Block_createGlobal :: proc (user_data: rawptr, user_proc: proc "c" (user_data: rawptr), allocator : mem.Allocator) -> (^Block, mem.Allocator_Error) {
...
Block_createGlobalWithParam :: proc (user_data: rawptr, user_proc: proc "c" (user_data: rawptr, t: $T), allocator : mem.Allocator) -> (^Block, mem.Allocator_Error) {
...
internal_block_literal_make :: proc (is_global: bool, user_data: rawptr, user_proc: rawptr, invoke: rawptr, allocator: mem.Allocator) ->  (b: ^Block, err: mem.Allocator_Error) {
	...
	if is_global {
		...
		return auto_cast copy_slice(cast(^Copying(Block))(&bl)), .None
	}
}
