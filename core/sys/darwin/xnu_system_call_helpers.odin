...
sys_open :: proc(path: string, oflag: Open_Flags, mode: Permission) -> (c.int, bool) {
    ...
    cmode: u32 = 0
    cflags: u32 = 0
    cpath: cstring = strings.cstring_clone_from_string(path, runtime.temp_allocator)
...
sys_rename :: proc(path: string, new_path: string) -> bool {
    runtime.TEMP_ALLOCATOR_TEMP_GUARD()
    cpath: cstring = strings.cstring_clone_from_string(path, runtime.temp_allocator)
    cnpath: cstring = strings.cstring_clone_from_string(new_path, runtime.temp_allocator)
...
sys_shm_open :: proc(name: string, oflag: Open_Flags, mode: Permission) -> (c.int, bool) {
    ...
    cmode: u32 = 0
    cflags: u32 = 0
    cname: cstring = strings.cstring_clone_from_string(name, runtime.temp_allocator)
