...
Open_Flags_Enum :: enum u8 {
    ...
    NONBLOCK, /* no delay */
    APPEND, /* set dyn_array.append mode */
...
@(private)
strings.cstring_clone_from_string :: proc(s: string, allocator: mem.Allocator, loc := #caller_location) -> cstring {
    c := slice.create([]byte, len(s)+1, allocator, loc)
    slice.copy(c, s)
    c[len(s)] = 0
    return cstring(&c[0])
}
...
sys_open :: proc(path: string, oflag: Open_Flags, mode: Permission) -> (c.int, bool) {
    ...
    cmode: u32 = 0
    cflags: u32 = 0
    cpath: cstring = strings.cstring_clone_from_string(path, allocators.temp_allocator)
...
sys_rename :: proc(path: string, new_path: string) -> bool {
    runtime.TEMP_ALLOCATOR_TEMP_GUARD()
    cpath: cstring = strings.cstring_clone_from_string(path, allocators.temp_allocator)
    cnpath: cstring = strings.cstring_clone_from_string(new_path, allocators.temp_allocator)
...
sys_shm_open :: proc(name: string, oflag: Open_Flags, mode: Permission) -> (c.int, bool) {
    ...
    cmode: u32 = 0
    cflags: u32 = 0
    cname: cstring = strings.cstring_clone_from_string(name, allocators.temp_allocator)
