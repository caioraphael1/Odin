...
stirling :: proc(x: f64) -> (f64, f64) {
    ...
    if x > MAX_STIRLING { // avoid pow() overflow
        v := pow_f64(x, 0.5*x-0.25)
        y1, y2 = v, v/y1
    } else {
        y1 = pow_f64(x, x-0.5) / y1
    }
...
gamma_f64 :: proc(x: f64) -> f64 {
    is_neg_int_f64 :: proc(x: f64) -> bool {
        if x < 0 {
            _, xf := modf_f64(x)
            return xf == 0
        }
    ...
    switch {
    case is_neg_int_f64(x) || is_inf_f64(x, -1) || is_nan_f64(x):
        return nan()
    case is_inf_f64(x, 1):
        return inf(1)
    case x == 0:
...
gamma_f16   :: proc(x: f16)   -> f16   { return f16(  gamma_f64(f64(x))) }
gamma_f16le :: proc(x: f16le) -> f16le { return f16le(gamma_f64(f64(x))) }
gamma_f16be :: proc(x: f16be) -> f16be { return f16be(gamma_f64(f64(x))) }
gamma_f32   :: proc(x: f32)   -> f32   { return f32(  gamma_f64(f64(x))) }
gamma_f32le :: proc(x: f32le) -> f32le { return f32le(gamma_f64(f64(x))) }
gamma_f32be :: proc(x: f32be) -> f32be { return f32be(gamma_f64(f64(x))) }
gamma_f64le :: proc(x: f64le) -> f64le { return f64le(gamma_f64(f64(x))) }
gamma_f64be :: proc(x: f64be) -> f64be { return f64be(gamma_f64(f64(x))) }
