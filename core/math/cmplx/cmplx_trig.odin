...
cos_complex128 :: proc(x: complex128) -> complex128 {
    ...
    switch re, im := real(x), imag(x); {
    case im == 0 && (math.is_inf(re, 0) || math.is_nan(re)):
        return complex(math.nan(), -im*math.copy_sign(0, re))
    case math.is_inf(im, 0):
        switch {
        case re == 0:
            return complex(math.inf(1), -re*math.copy_sign(0, im))
        case math.is_inf(re, 0) || math.is_nan(re):
...
cosh_complex128 :: proc(x: complex128) -> complex128 {
    ...
    switch re, im := real(x), imag(x); {
    case re == 0 && (math.is_inf(im, 0) || math.is_nan(im)):
        return complex(math.nan(), re*math.copy_sign(0, im))
    case math.is_inf(re, 0):
        switch {
        case im == 0:
            return complex(math.inf(1), im*math.copy_sign(0, re))
        case math.is_inf(im, 0) || math.is_nan(im):
            return complex(math.inf(1), math.nan())
        }
    case im == 0 && math.is_nan(re):
