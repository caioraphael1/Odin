...
float64_gamma :: proc(alpha, beta: f64, gen: runtime.Random_Generator) -> f64 {
    ...
    case:
        ...
        for {
            ...
            p := b * u
            if p <= 1 {
                x = math.pow_f64(p, 1/alpha)
            } else {
                x = -math.ln((b - p) / alpha)
            }
            u1 := float64(gen)
            if p > 1 {
                if u1 <= math.pow_f64(x, alpha-1) {
                    break
                }
            } else if u1 <= math.exp_f64(-x) {
                break
            }
        }
        return x * beta
...
float64_von_mises :: proc(mean_angle, kappa: f64, gen: runtime.Random_Generator) -> f64 {
    ...
    for {
        u1 := float64(gen)
        z = math.cos_f64(math.TAU * 0.5 * u1)

        d := z / (t + z)
        u2 := float64(gen)
        if u2 < 1 - d*d || u2 <= (1-d)*math.exp_f64(d) {
            break
        }
    }
    ...
    u3 := float64(gen)
    if u3 > 0.5 {
        return math.mod_f64(mu + math.acos(f), math.TAU)
    } else {
        return math.mod_f64(mu - math.acos(f), math.TAU)
    }
}
...
float64_cauchy_lorentz :: proc(x_0, gamma: f64, gen: runtime.Random_Generator) -> f64 {
    ...
    return math.tan_f64(math.PI * (float64(gen) - 0.5))*gamma + x_0
}
// Cauchy-Lorentz Distribution
// `x_0` is the location, `gamma` is the scale where `gamma` > 0
