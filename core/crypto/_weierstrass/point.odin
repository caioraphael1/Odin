...
pt_identity :: proc(p: ^$T) {
	fe_zero(&p.x)
	fe_one(&p.y)
	fe_zero(&p.z)
}

pt_generator :: proc(p: ^$T) {
	fe_gen_x(&p.x)
	fe_gen_y(&p.y)
	fe_one(&p.z)
}
...
@(private)
pt_add_mixed :: proc(p, a: ^$T, x2, y2: ^$U) {
...
pt_negate :: proc(p, a: ^$T) {
	fe_set(&p.x, &a.x)
	fe_negate(&p.y, &a.y)
	fe_set(&p.z, &a.z)
}

pt_rescale :: proc(p, a: ^$T) {
	// A = 1/Z1
	// X3 = A*X1
	// Y3 = A*Y1
	// Z3 = 1
...
pt_cond_select :: proc(p, a, b: ^$T, ctrl: int) {
...
pt_is_identity :: proc(p: ^$T) -> int {
	return fe_is_zero(&p.z)
}


pt_is_y_odd :: proc(p: ^$T) -> int {
	tmp: T
	defer pt_clear(&tmp)
...
@(private)
set_yy_candidate :: proc(maybe_yy, x: ^$T) {
	// RHS: x^3 + ax + b
	rhs, tmp: T
