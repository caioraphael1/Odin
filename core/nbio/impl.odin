...
_tick_ :: proc(l: ^Event_Loop, timeout: time.Duration) -> (err: General_Error) {
...
_read_entire_file :: proc(l: ^Event_Loop, path: string, user_data: rawptr, cb: Read_Entire_File_Callback, allocator: mem.Allocator, dir := CWD) {
    open_poly3(path, user_data, cb, allocator, on_open, dir=dir, l=l)

    on_open :: proc(op: ^Operation, user_data: rawptr, cb: Read_Entire_File_Callback, allocator: mem.Allocator) {
        if op.open.err != nil {
    ...
    on_stat :: proc(op: ^Operation, user_data: rawptr, cb: Read_Entire_File_Callback, allocator: mem.Allocator) {
        if op.stat.err != nil {
        ...
        buf, err := slice_create([]byte, op.stat.size, allocator)
        if err != nil {
            close(op.stat.handle)
    ...
    on_read :: proc(op: ^Operation, user_data: rawptr, cb: Read_Entire_File_Callback, allocator: mem.Allocator) {
...
bufs_init :: proc(bufs: ^Bufs, orig: ^[][]byte, allocator: mem.Allocator) -> mem.Allocator_Error {
    if len(orig) > 1 {
        backing := slice_create([][]byte, len(orig)*2, allocator) or_return
