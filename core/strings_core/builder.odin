...
builder_replace :: proc(b: ^Builder, old, new: string, n: int, loc := #caller_location) -> (replaced: int, err: mem.Allocator_Error) {
    ...
    if len(old) == 0 {
        for i := 0; i <= len(b.buf); i += len(new)+1 {
            ...
            dyn_array.resize(&b.buf, len(b.buf)+len(new), loc) or_return
            slice.copy(b.buf[i+len(new):], b.buf[i:])
            slice.copy_from_string(b.buf[i:], new)
            replaced += 1
        }
    } else {
        for i := 0; i < len(b.buf); /**/ {
            ...
            if len(new) >= len(old) {
                dyn_array.resize(&b.buf, len(b.buf) + len(new)-len(old)) or_return
            }
            ...
            dst := cur[len(new):]
            slice.copy(dst, src)
            slice.copy_from_string(cur, new)
