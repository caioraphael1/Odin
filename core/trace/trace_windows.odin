...
_resolve :: proc(ctx: ^Context, frame: Frame, allocator: mem.Allocator) -> (fl: Frame_Location) {
	...
	if win32.SymFromAddrW(ctx.impl.hProcess, win32.DWORD64(frame), &{}, symbol) {
		fl.procedure, _ = win32.wstring_to_utf8_alloc(cstring16(&symbol.Name[0]), -1, allocator)
	} else {
	...
	if win32.SymGetLineFromAddrW64(ctx.impl.hProcess, win32.DWORD64(frame), &{}, &line) {
		fl.file_path, _ = win32.wstring_to_utf8_alloc(line.FileName, -1, allocator)
