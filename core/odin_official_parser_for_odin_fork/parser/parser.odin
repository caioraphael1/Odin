...
parse_file :: proc(p: ^Parser, file: ^ast.File) -> bool {
    ...
    for p.curr_tok.kind != .Package && p.curr_tok.kind != .EOF {
        if p.curr_tok.kind == .Comment {
            ...
            if !token_that_is_not_comment_or_tag_found {
                ...
                first_token_that_is_not_comment_or_tag = p.curr_tok
                break
            }
    ...
    if invalid_tag_found {
        error(p, {}, "Expected only comments or file tags with '#+' before any other token");
        return false;
    }
    ...
    get_last_element_in_path :: proc(s: string) -> string {
        i := len(s) - 1
        for ; i > 0; i -= 1 {
            if s[i] == '/' || s[i] == '\\' {
                break;
    ...
    package_name := get_last_element_in_path(directory_from_path(p.file.fullpath))

    p.file.pkg_name = package_name

    pd := ast.new(ast.Package_Decl, {}, end_pos(p.prev_tok))
    pd.docs    = docs
    pd.token   = p.file.pkg_token
    pd.name    = package_name
