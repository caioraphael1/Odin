...
parse_or_exit :: proc(
    ...
    if error != nil {
        ...
        if len(args) == 0 {
            ...
            write_usage(stderr, T, program, style, allocator)
            fmt.wprintln(stderr)
        }

        print_errors(T, error, program, style, allocator)
...
*/
@(optimization_mode="favor_size")
print_errors :: proc(data_type: typeid, error: Error, program: string, style: Parsing_Style = .Odin, allocator: mem.Allocator) {
    stderr := os.to_stream(os.stderr)
    stdout := os.to_stream(os.stdout)

    switch specific_error in error {
    case Parse_Error:
        _ = fmt.wprintfln(stderr, "[%T.%v] %s", specific_error, specific_error.reason, specific_error.message)
    case Open_File_Error:
        _ = fmt.wprintfln(stderr, "[%T#%i] Unable to open file with perms 0o%o in mode 0x%x: %s",
            specific_error,
    ...
    case Validation_Error:
        _ = fmt.wprintfln(stderr, "[%T] %s", specific_error, specific_error.message)
    case Help_Request:
        write_usage(stdout, data_type, program, style, allocator)
    }
}
/*
