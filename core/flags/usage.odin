...
write_usage :: proc(out: io.Writer, data_type: typeid, program: string = "", style: Parsing_Style = .Odin, allocator: mem.Allocator) {
    ...
    defer _ = dyn_array_delete(visible_flags)
    ...
    for field in reflect.struct_fields_zipped(data_type) {
        ...
        _ = dyn_array_append(&visible_flags, flag)
    }
    ...
    for flag, i in visible_flags {
        if i == divider_index {
            ...
            strings.write_byte(&builder, '\t')
            spacing, _ := strings.repeat(" ", SPACING + longest_flag_length, runtime.temp_allocator)
