...
iana_to_windows_tz :: proc(iana_name: string, allocator: mem.Allocator) -> (name: string, success: bool) {
    ...
    wintz_name, err := windows.utf16_to_utf8_alloc(wintz_name_buffer[:wintz_name_len], allocator)
    if err != nil {
        return
    }
...
local_tz_name :: proc(allocator: mem.Allocator) -> (name: string, success: bool) {
    ...
    iana_name, err := windows.utf16_to_utf8_alloc(iana_name_buffer[:zone_str_len], allocator)
    if err != nil {
        return
    }
...
generate_rrule_from_tzi :: proc(tzi: ^REG_TZI_FORMAT, abbrevs: TZ_Abbrev, allocator: mem.Allocator) -> (rrule: datetime.TZ_RRule, ok: bool) {
    ...
    if err != nil { return }
    defer if err != nil { _ = strings.string_delete(std_name, allocator) }
    ...
    if err != nil { return }
    defer if err != nil { _ = strings.string_delete(dst_name, allocator) }
...
_region_load :: proc(reg_str: string, allocator: mem.Allocator) -> (out_reg: ^datetime.TZ_Region, success: bool) {
    ...
    if reg_str == "local" {
        ...
        if !ok {
            _ = strings.string_delete(iana_name, allocator)
            return
        }
    } else {
    ...
    defer _ = strings.string_delete(wintz_name, allocator)
    defer _ = strings.string_delete(iana_name, allocator)
    ...
    if res != 0 { return }
    defer _ = windows.RegCloseKey(key)
