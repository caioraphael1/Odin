...
rfc3339_to_time_and_offset :: proc(rfc_datetime: string, is_leap: ^bool = nil) -> (res: Time, utc_offset: int, consumed: int) {
    ...
    if _res, ok := components_to_time(moment.year, moment.month, moment.day, moment.hour, moment.minute, moment.second, moment.nano); !ok {
        return {}, 0, 0
    } else {
...
time_to_rfc3339 :: proc(time: Time, utc_offset : int = 0, include_nanos := true, allocator: mem.Allocator) -> (res: string, ok: bool) {
    ...
    res_as_slice, res_alloc := slice.create([]u8, len=offset, allocator = allocator)
    if res_alloc != nil {
        return "", false
    }
